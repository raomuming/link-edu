...
APIServer.prototype._register_for_all_routes = function() {
	...
	this.express.use(function(request, response, next) {
		if (request._expectHeaderReceived) {
			// Expect/Continue case
			// stub body and run bodyParser when we actually recieve body
			request.body = {};
			next();
		} else {
			bodyParserMiddleware(request, response, next);
		}
	});
};
...
APIServer.prototype._register_models_routes = function() {
	let self = this;
	Routes.register_models_routes(self.express);
};
